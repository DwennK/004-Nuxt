...
describe('print frame boundaries', () => {
  it.each(['/documents/1/print?profile=a4', '/documents/42/print?profile=thermal', '/dossiers/1/print/'])('allows only same-origin embedding of %s', async (path) => {
  ...
  it.each(['/', '/login', '/documents/1', '/dossiers/1/edit', '/api/documents/1/pdf', '/documents/1/print/other'])('continues to block all embedding of %s', async (path) => {
    const result = await headers(path)
