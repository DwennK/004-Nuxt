...
test('reservation, takeover, preserved draft, save, reminders and mobile layout', async ({ page, context }, testInfo) => {
  ...
  await expect(page.getByRole('button', { name: 'Copier les saisies' })).toBeEnabled()
  ...
  await page.getByRole('textbox', { name: 'Description de la panne', exact: false }).fill('Brouillon pour le rappel sonore')
  await expect(page.getByTestId('unsaved-changes')).toBeVisible()
  await page.getByRole('button', { name: 'Couper le rappel', exact: true }).click()
  await page.clock.fastForward(300_001)
  await expect(page.getByRole('button', { name: 'Rappel coupé', exact: true })).toBeDisabled()
...
test('an explicitly retried payment with a lost response is recorded only once', async ({ page, context }) => {
  ...
  await page.getByRole('button', { name: 'Paiement détaillé', exact: true }).click()
  ...
  await page.getByRole('button', { name: 'Encaisser · Espèces', exact: true }).click()
