...
{
  const paymentWriter = read('server/utils/pos/payment-writes.ts')
  ...
  assertIncludes(documents, 'calculateCommercialTotals as calculateDocumentTotals', 'server uses the shared commercial calculation kernel')
  ...
  assertIncludes(payments, 'getPayablePaymentDocument(tx, input.documentId)', 'payment writes validate payable document type inside their transaction')
  ...
  assertIncludes(payments, 'recordDocumentPayment(tx, document,', 'payment creation delegates to the shared transactional writer')
  assertIncludes(paymentWriter, 'assertPaymentFitsDocument(tx, document, input.amount)', 'payment creation validates the remaining balance')
  assertIncludes(paymentWriter, 'syncDocumentStatus(document.id, tx)', 'payment creation syncs document status inside its transaction')
  assertIncludes(paymentWriter, 'isPayableDocumentType(document.type)', 'the shared writer rejects non-payable document types')
  assertIncludes(paymentWriter, 'documentStatus: document.status', 'the shared writer applies the cancellation policy to the current document')
  assertIncludes(documents, 'recordDocumentPayment(tx, document, { ...input, status: \'paid\' })', 'mark-paid uses the shared payment policy')
  ...
  assertIncludes(documents, 'recordDocumentPayment(tx, createdDocument,', 'create-and-pay syncs status through the transactional writer')
  ...
  assertIncludes(tickets, 'createQuoteFromTicket(ticketId: number, idempotencyKey: string, dossier?: DossierWriteContext)', 'ticket quote creation requires an idempotency key')
  assertIncludes(tickets, 'createCustomerOrderFromTicket(ticketId: number, idempotencyKey: string, dossier?: DossierWriteContext)', 'ticket order creation requires an idempotency key')
  assertIncludes(tickets, 'createInvoiceFromTicket(ticketId: number, idempotencyKey: string, dossier?: DossierWriteContext)', 'ticket invoice creation requires an idempotency key')
  assertIncludes(documentCreateRoute, 'createDocumentRecord(body, { key: idempotencyKey, dossier: readDossierWriteContext(event) })', 'document route forwards its idempotency key')
  assertIncludes(createAndPayRoute, 'createAndPayDocumentRecord(body.document, body.payment, idempotencyKey, readDossierWriteContext(event))', 'create-and-pay route delegates to the atomic idempotent domain operation')
  assertIncludes(ticketQuoteRoute, 'createQuoteFromTicket(params.id, idempotencyKey, readDossierWriteContext(event))', 'ticket quote route forwards its idempotency key')
  assertIncludes(ticketOrderRoute, 'createCustomerOrderFromTicket(params.id, idempotencyKey, readDossierWriteContext(event))', 'ticket order route forwards its idempotency key')
  assertIncludes(ticketInvoiceRoute, 'createInvoiceFromTicket(params.id, idempotencyKey, readDossierWriteContext(event))', 'ticket invoice route forwards its idempotency key')
