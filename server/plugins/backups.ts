...
export default defineNitroPlugin((nitroApp) => {
  nitroApp.hooks.hook('cloudflare:scheduled', async ({ controller, env }) => {
    ...
    const { client, dropbox, turso } = backupContext(undefined, env)
    try {
      ...
      await runBackup(client, dropbox, 'scheduled', turso, new Date(controller.scheduledTime).toISOString().slice(0, 10))
    } finally {
      client.close()
    }
  })
